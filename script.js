 ...
 */
const VIP_COUPONS = {
    "MUHLIS10": { percent: 10, maxUses: 50, active: true },
    "KIPAS15":  { percent: 15, maxUses: 30, active: true },
    "VIP20":    { percent: 20, maxUses: 10, active: true }
};
...
async function fetchAllVipOrdersFromFirebase() {
    ...
    try {
        ...
        const list = Object.keys(val)
            ...
            .filter(o => !!(o.proofImage || o.hasProof) && !o.deleted);
...
async function deleteVipOrderHard(orderId) {
    ...
    if (!gcDb || !gcReady) {
        showToast("Gagal", "Firebase belum siap", "error");
        return false;
    }
    if (!isCurrentUserAdmin()) {
        showToast("Admin only", "Masuk grup dulu pakai nama admin", "warning");
        return false;
    }
    if (!window.confirm("Hapus order " + orderId + " dari inbox admin?")) {
        return false;
    }

    // Soft-delete dulu (paling stabil di semua rules)
    try {
        await gcDb.ref("ffkipas_vip_orders/" + orderId).update({
            archived: true,
            deleted: true,
        ...
        console.warn("soft delete", e);
        showToast("Gagal", "Tidak bisa hapus order (cek Rules Firebase)", "error");
        return false;
    }

    // Coba hapus permanen di background — gagal pun diabaikan (order sudah hilang dari inbox)
    try {
...
async function renderVipOrdersList() {
    ...
    if (admin) {
        ...
        vipOrdersList.innerHTML = filtered.map(o => {
            ...
            const deleteBtn = `<button type="button" class="voi-copy voi-delete" data-delete="${escapeHtml(id)}" title="Hapus"><i class="fa-solid fa-trash"></i> Hapus</button>`;
